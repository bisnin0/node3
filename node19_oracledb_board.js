...
app.get('/list',(req, res)=>{
	...
	//실행하기        쿼리문, 콜백함수(에러, 선택한 레코드)
	conn.execute(sql, (error, result)=>{
		if(error){
			console.log("레코드 선택에러 발생 -- >"+ error);
		}else{
			var cnt = result.rows.length;//총레코드 수
			
			//list.ejs파일을 읽은후 선택한 레코드 렌더하여 클라이언트에게 보낸다.
...
app.post('/writeOk',(request, response)=>{
	...
	//      192.168.0.209:10016/   127.0.0.1:10016/
	var ip = requestip.getClientIp(request).substring(7);  
	console.log("ip-->"+ ip);  // ::ffff:192.168.0.209
...
app.get('/del',(req, res)=>{
	...
	conn.execute(sql, (error, result)=>{
		if(error){
			...
			res.writeHead(200, {"Content-Type":"text/html;charset=utf-8"});
			res.end("<script>alert('삭제되었습니다.');location.href='/list';</script>");
		}
	});
